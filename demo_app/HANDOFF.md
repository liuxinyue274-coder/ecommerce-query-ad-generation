...
本目录 `D:\hllm visualization\` 是一个**纯前端可视化看板**，用于：
...
**重要边界**：本目录**不修改任何 V5 主项目代码**。V5 主代码位于 `D:\hllm_env\KuaiSearch-main\`，看板只通过 import + 调用的方式使用它，前后端职责清晰。
...
- Python 3.9+
- V5 主项目已就位：`D:\hllm_env\KuaiSearch-main\`（路径可通过环境变量 `KS_PROJECT_ROOT` 覆盖）
...
- **不动 V5 主代码**：`D:\hllm_env\KuaiSearch-main\ad_generation\` 内任何文件均不修改。需要新能力时在看板侧（`app.py`）加封装层。
...
- V5 主项目交接文档：`D:\hllm_env\KuaiSearch-main\V5_HANDOFF_README.md`
