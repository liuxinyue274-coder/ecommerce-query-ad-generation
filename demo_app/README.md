...
看板**不修改**项目主代码，只通过 `sys.path` 引入 `D:\hllm_env\KuaiSearch-main\ad_generation` 下的模块：
...
> 看板本身只额外依赖 `streamlit`。其余 V5 链路需要的依赖（如 numpy/torch、`requests` 等）应该已经在 `KuaiSearch-main` 那边装好。如果直接 import 报 `ModuleNotFoundError`，按报错信息补装即可。

### 2. 启动

**方式一：双击 `run.bat`**（推荐 Windows 用户）

如果 KuaiSearch-main 不在默认的 `D:\hllm_env\KuaiSearch-main`，先用记事本打开 `run.bat` 改 `KS_PROJECT_ROOT`。

**方式二：命令行**

```bash
cd "D:\hllm visualization"
...
| 变量 | 说明 | 默认 |
| --- | --- | --- |
| `KS_PROJECT_ROOT` | KuaiSearch-main 的根路径 | `D:\hllm_env\KuaiSearch-main` |
...
- **看板字段位置**全部对齐 `V5_HANDOFF_README.md` 第 8 节的字段路径，包括 `result.intent_enricher` / `result.evidence_selector.selected_evidence_items` / `result.copy_ranker.ranked_candidates` / `result.final_ad_copy.final_ad_copy` 等。
...
| 离线浏览找不到 JSONL | 先在 KuaiSearch-main 跑一次 `python -X utf8 ad_generation/demo_v5_pipeline.py --limit 5 --provider local_fake` 生成 |
